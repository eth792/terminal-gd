...
<!-- 此章节由 `npm run update-docs` 自动生成 -->
<!-- 手工编辑的内容在下面或独立文件中风险较低 -->
...
- 21 个案例因供应商字段提取错误导致 f1_score=0 或极低
- 修复后预期 +5-7% 自动通过率（10倍 ROI 机会）

**测试运行包**: `runs/run_v0.1.6_full_20251113_214123/` (完整测试, 36.6 分钟)
...
**目的**：在恢复 session 前验证关键环境组件，快速发现配置问题。

**验证清单**：

```bash
# 1. 验证 spec workflow 目录存在
ls -la .spec-workflow/specs/ 2>/dev/null && echo "✅ Spec workflow 目录存在" || echo "❌ Spec workflow 目录不存在"

# 2. 验证关键文档存在
[ -f docs/PROJECT_STATUS.md ] && [ -f docs/implementation_record.md ] && [ -f CLAUDE.md ] && \
  echo "✅ 关键文档完整" || echo "❌ 关键文档缺失"

# 3. 检查 Git 分支和状态
git branch --show-current && echo "✅ Git 正常" || echo "❌ Git 异常"

# 4. MCP 工具（自动检测，无需手动验证）
# Claude Code 启动时自动加载 MCP servers
```

**成功标准**：
- ✅ `.spec-workflow/specs/` 目录存在且可访问
- ✅ 三个核心文档（PROJECT_STATUS.md, implementation_record.md, CLAUDE.md）存在
- ✅ Git 在正常分支（非 detached HEAD 状态）
- ✅ MCP tools 显示在 Claude Code 工具列表中

**故障排查**：
- ❌ **specs/ 不存在** → 首次使用需初始化：运行 `mcp__spec-workflow__spec-workflow-guide` 工具
- ❌ **文档缺失** → 检查是否在正确的项目根目录（应有 `package.json` 和 `docs/` 文件夹）
- ❌ **Git detached HEAD** → 切换到工作分支：`git checkout main` 或 `git checkout <branch-name>`
- ❌ **MCP tools 未加载** → 重启 Claude Code 或检查 `~/.config/claude/mcp.json` 配置
...
<!-- ⚠️ AUTOGENERATED SECTION - DO NOT EDIT MANUALLY -->
<!-- 此章节由 `npm run update-docs` 的脚本自动生成 -->
<!-- 所有快速状态恢复章节的数值都会自动更新 -->
<!-- 描述性内容仍需手动维护以保持准确性 -->

**此文档的以下章节会自动生成：**
- 🚀 快速状态恢复（顶部的元数据、KPI表格、最近完成工作的基本信息）
- 📊 PROJECT_STATUS.md 中的KPI表格
- 📝 docs/implementation_record.md 中的版本条目

**以下章节需要手动维护：**
- 关键技术决策记录
- 快速恢复步骤中的高级技巧
- 版本发布工作流的具体要求
- CLAUDE.md 中的代码变更详情和关键发现
