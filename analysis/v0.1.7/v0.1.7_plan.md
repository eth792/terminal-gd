...
| EXTRACT_EMPTY_SUPPLIER | 0 | 27 | +27 | +∞ |
...
1. **endsWithEntity 检查被误解**
   - 原设计意图：防止拼接其他字段的内容（不仅仅是带标签的行）
   - v0.1.7 修改：移除此检查，认为 `hasOtherLabel` 标签检查已足够
   - 实际后果：供应商名称盲目拼接下一行的工程名称内容（未带标签）
   - 典型错误：`"武汉星火电线电缆有限公司武汉锦府置有限公司新希望华中区"` (两家公司名称合并)

2. **分隔符检查过于严格**
   - 添加的检查：标签后必须紧跟 `[:：=\s/]` 才视为合法
   - 实际后果：OCR 错误导致分隔符缺失时（如"供应商武汉..."），无法提取
   - 导致 27 个新的 EXTRACT_EMPTY_SUPPLIER 案例

3. **跨字段排除机制不完整**
   - 实现逻辑：检查下一行是否包含其他字段的标签
   - 实际问题：无法检测**不带标签的字段内容**（如工程名称的续行）
   - 需要结合内容特征（如 endsWithEntity）与标签检测

4. **配置清理未经验证**
   - 移除了"供应单位名称"、"采购订单供应商"等标签
   - 未先 grep 验证这些标签在实际 OCR 文本中的出现频率
   - 可能导致某些合法标签无法匹配

### 关键教训

1. **不要一次修改多个关键逻辑** - 无法定位具体失败原因
2. **理解现有逻辑的真实意图** - endsWithEntity 是内容特征检测，不是简单的"Bug"
3. **配置清理需要数据支撑** - 先 grep 验证，再决定移除
4. **分隔符检查不能过严** - 必须容忍 OCR 错误
5. **增量测试是王道** - 每次修改后立即验证，不要等完整测试
...
**回滚文件**:
- `configs/v0.labs/10dae06c/label_alias.json` - 恢复 v0.1.6 配置
- `packages/ocr-match-core/src/extract/extractor.ts` - 恢复 v0.1.6 提取逻辑

**验证**: 代码已恢复到 commit `14a5eb79` 之前的状态

### 下一步建议

**v0.1.7 重新实施（保守策略）**:
1. 创建 10-20 个代表性样本的小测试集
2. 每次只修改一个点，立即测试验证
3. 优先级调整：
   - P1: 修复配置污染（但保留可能有效的标签，先 grep 验证）
   - P2: 改进 endsWithEntity（不是移除，而是增强）
   - P3: 分隔符检查放宽（容忍无分隔符的情况）
   - P4: 跨字段排除增强（结合标签检测和内容特征）

**详细失败分析**: 参见 `analysis/v0.1.7/v0.1.7_failure_analysis.md`
