...
- OCR 归一化补充（"工理"→"工程"）
- 目标测试策略验证（节省 12-15 小时）
- **发现关键问题**：50% 失败案例源于提取逻辑错误

**预期效果**: Exact **70 → 71** (+1, +1.4%), 自动通过率 **31.5% → 31.9%** (+0.4%)
...
**测试策略**: 仅测试 22 个 FIELD_SIM_LOW_SUPPLIER 案例（节省 12-15 小时）

| 版本 | Exact | Review | Fail | 自动通过率 | 测试策略 |
|------|-------|--------|------|------------|---------|
| v0.1.5 | 70 (31.5%) | 25 (11.3%) | 127 (57.2%) | 31.5% | 完整测试 (222 样本) |
| **v0.1.6 (预期)** | **71 (32.0%)** | 24 (10.8%) | 127 (57.2%) | **31.9%** | 目标测试推断 |
